...
class ReviewerState(rx.State):
    ...
    def _severity_for_issue(issue: str) -> str:
        text = issue.lower()
        critical_terms = ["vulnerability", "security", "injection", "xss", "critical", "rce", "out-of-bounds"]
        high_terms = ["error", "undefined", "failed", "exception", "missing", "not found", "syntax"]
        medium_terms = ["unused", "style", "complexity", "performance", "warning", "lint"]
        low_terms = ["best practice", "readability", "documentation", "comment"]
    ...
    def severity_breakdown(self) -> list[dict]:
        ...
        for issue in self.issues_found:
            counts[self._severity_for_issue(issue)] += 1
        return [
            {
                "severity": s,
                "count": str(counts[s]),
                **self._severity_style(s)
            }
    ...
    def grouped_external_issues(self) -> list[str]:
        external_terms = ["eslint", "tsc", "golangci", "go vet", "rustc", "javac", "compiler"]
        return [issue for issue in self.issues_found if any(t in issue.lower() for t in external_terms)]

    @rx.var
    def grouped_static_issues(self) -> list[str]:
        external_terms = ["eslint", "tsc", "golangci", "go vet", "rustc", "javac", "compiler"]
        static_terms = ["unused", "undefined", "style", "lint", "syntax"]
        return [
            issue for issue in self.issues_found
            if not any(t in issue.lower() for t in external_terms)
            and any(t in issue.lower() for t in static_terms)
        ]
    ...
    def grouped_ai_issues(self) -> list[str]:
        ...
        return [
            issue for issue in self.issues_found
            if not any(t in issue.lower() for t in external_terms)
            and not any(t in issue.lower() for t in static_terms)
        ]
    ...
    def original_code_with_lines(self) -> str:
        return "\n".join(f"{idx + 1:>3} | {line}" for idx, line in enumerate(self.original_code.splitlines()))

    @rx.var
    def improved_code_with_lines(self) -> str:
        return "\n".join(f"{idx + 1:>3} | {line}" for idx, line in enumerate(self.improved_code.splitlines()))

    def _build_report_text(self) -> str:
        issues_text = "\n".join(f"- {item}" for item in self.issues_found) or "- None"
        return (
    ...
    def _build_pdf_report_bytes(self, report_text: str) -> bytes:
        ...
        margin_x, margin_top, margin_bottom, chars_per_line = 42, 50, 42, 100
    ...
    def download_pdf_report(self):
        if not self.original_code.strip() and not self.improved_code.strip():
            ...
            return
        pdf_bytes = self._build_pdf_report_bytes(self._build_report_text())
    ...
    def analyze(self) -> None:
        ...
        self.unused_imports = [f"{i.get('name', '?')} (from {i.get('module', '?')}) - line {i.get('line', '?')}" for i in static.get("unused_imports", [])]
        self.unused_functions = [f"{i.get('name', '?')} - line {i.get('line', '?')}" for i in static.get("unused_functions", [])]
        self.unused_variables = [f"{i.get('name', '?')} - line {i.get('line', '?')}" for i in static.get("unused_variables", [])]
        self.style_violations = [f"{i.get('code', '?')} - {i.get('message', '?')} (line {i.get('line', '?')}, col {i.get('column', '?')})" for i in static.get("style_violations", [])]
        self.external_linter_violations = [f"{i.get('tool', '?')} {i.get('code', '?')} - {i.get('message', '?')} (line {i.get('line', '?')}, col {i.get('column', '?')})" for i in static.get("external_linter_violations", [])]
        self.external_linter_tool_status = [f"{i.get('tool', '?')}: {'available' if i.get('available', False) else 'not available'} | issues: {i.get('issues', 0)}{(' | ' + str(i.get('note'))) if i.get('note') else ''}" for i in static.get("external_linter_tool_status", [])]

        vc = result.get("variable_context", {})
        self.created_variables = [f"{i.get('name', '?')} - line {i.get('line', '?')}" for i in vc.get("created", [])]
        self.used_variables = [f"{i.get('name', '?')} - line {i.get('line', '?')}" for i in vc.get("used", [])]

        snippet = self.code_input.strip().replace("\n", " ")[:80]
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M")
        record_id = datetime.now().strftime("%Y%m%d%H%M%S%f")
        self.history_entries.insert(0, {
            "id": record_id, "summary": f"{timestamp} | {self.selected_language} | Grade: {self.quality_grade} | Issues: {self.issues_count} | {snippet}",
            "timestamp": timestamp, "language": self.selected_language, "grade": self.quality_grade, "issues_count": str(self.issues_count),
            "analysis_summary": self.analysis_summary, "original_code": self.original_code, "improved_code": self.improved_code, "issues_found": "\n".join(self.issues_found)
        })
    ...
    def toggle_history_entry(self, record_id: str) -> None:
        self.selected_history_id = "" if self.selected_history_id == record_id else record_id
    ...
    def send_assistant_message(self) -> None:
        ...
        chat_history = [{"role": i.get("role", ""), "content": i.get("content", "")} for i in self.assistant_messages if i.get("role") in ("user", "assistant")]
        code_context = str({"quality_grade": self.quality_grade, "issues_count": self.issues_count, "analysis_summary": self.analysis_summary, "issues_found": self.issues_found, "original_code": self.original_code, "improved_code": self.improved_code})
        response = ask_ai_assistant(prompt, code_context, chat_history)
        self.assistant_messages.extend([{"role": "user", "content": prompt}, {"role": "assistant", "content": str(response)}])
